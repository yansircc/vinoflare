import { ParsedArgs, CommandFlags } from '../types';
...
export function parseArgs(argv: string[] = process.argv): ParsedArgs {
  ...
  const projectName = args.find(arg => !arg.startsWith('--') && !arg.startsWith('-'));
  ...
  const flags: CommandFlags = {
    ...
    projectType: getArgValue(args, '--type') as 'full-stack' | 'api-only' | undefined,
