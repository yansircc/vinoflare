    ...
    async (c) => {
      try {
        ...
        const quotes = await db
          .select()
          .from(schema.quotes)
          .orderBy(sort === 'newest' ? desc(schema.quotes.createdAt) : asc(schema.quotes.createdAt))
          .limit(limit)
          .offset(offset)
    ...
    async (c) => {
      try {
        ...
        const stats = {
          ...
          averageMessageLength: allQuotes.length > 0 
            ? Math.round(allQuotes.reduce((sum, q) => sum + q.message.length, 0) / allQuotes.length)
            : 0
        }
