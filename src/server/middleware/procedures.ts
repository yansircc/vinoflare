...
export const optionalAuthMiddleware = createMiddleware<AuthContext>(async (c, next) => {
  try {
    ...
    // 可选认证失败时只记录日志，不阻止请求
    console.warn('🔓 可选认证失败:', error instanceof Error ? error.message : 'Unknown error');
    await next();
  }
})
...
export const loggingMiddleware = createMiddleware(async (c, next) => {
  ...
  console.log(`${logLevel} [${requestId}] ${c.req.method} ${c.req.path} - ${status} - ${duration.toFixed(2)}ms`)
})
