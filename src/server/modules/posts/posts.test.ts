...
describe("Posts Module", () => {
  ...
  describe("Authentication", () => {
    it("should require authentication for all endpoints", async () => {
      ...
      const endpoints = [
        ...
        { method: "POST", path: "/api/posts", body: JSON.stringify({ title: "Test" }) },
      ];
  ...
  describe("Concurrent operations", () => {
    it("should handle concurrent post creation", async () => {
      const titles = Array.from({ length: 5 }, (_, i) => ({ title: `Concurrent Post ${i}` }));
      ...
      const latestRequest = await createAuthenticatedRequest("/api/posts/latest");
