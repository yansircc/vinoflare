...
import { execute, executeSequence } from '../utils/exec';
...
export class ProjectInitProcessor extends BaseProcessor {
  ...
  shouldRun(context: ExecutionContext): boolean {
    return !context.config.skipInit && context.getState<boolean>('dependenciesInstalled') === true;
  }
  ...
  async process(context: ExecutionContext): Promise<void> {
    ...
    try {
      ...
      context.logger.warn('You can run the initialization commands manually later.');
    }
  }
