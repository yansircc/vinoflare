...
import { readFile, writeFile, pathExists } from '../utils/fs';
...
export class FileTransformProcessor extends BaseProcessor {
  ...
  async process(context: ExecutionContext): Promise<void> {
    ...
    for (const [featureName, featureConfig] of Object.entries(transformRules.features)) {
  ...
  private shouldProcessFeature(featureName: string, context: ExecutionContext): boolean {
  ...
  private async processTransformations(
    ...
    for (const transformation of transformations) {
      ...
      if (transformation.condition && !this.evaluateCondition(transformation.condition, context)) {
        continue;
      }

      if (!await pathExists(filePath)) {
        context.logger.debug(`File not found for transformation: ${transformation.file}`);
        continue;
      }
      ...
      try {
        ...
        context.logger.error(`Failed to transform ${transformation.file}: ${error}`);
      }
    }
  }
