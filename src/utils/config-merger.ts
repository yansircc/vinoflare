 ...
 */
export function mergeConfigWithRules(
 ...
 */
export function mergeTemplateInheritance(
		...
		if (existing) {
			...
			featureMap.set(feature.name, {
				...
				files: {
					...
					add: [...(existing.files?.add || []), ...(feature.files?.add || [])],
				},
				dependencies: {
...
export const ConfigMerger = {
	merge: mergeConfigWithRules,
	mergeInheritance: mergeTemplateInheritance,
};
