...
// Utils
import { detectPackageManager, isValidPackageManager } from './utils/package-manager';
...
async function main() {
  ...
  const shouldInit = shouldInstall ? await prompts.getInitOption(includeDb) : false;
  ...
  if (parsedArgs.flags.yes) {
    ...
    console.log(kleur.dim(`  Database: ${config.features.database ? 'Yes' : 'No'}`));
