...
export function updateApiRoutes(names: NameVariations) {
  ...
  const lastModuleImportMatch = apiContent.match(/import \w+Module from "\.\.\/modules\/\w+\/index";/g);
  if (lastModuleImportMatch) {
    ...
    apiContent = apiContent.slice(0, insertImportAt) + importStatement + "\n" + apiContent.slice(insertImportAt);
  } else {
    // If no module imports found, add after other imports
    const importEndMatch = apiContent.match(/import[\s\S]*?(?=\n\n|export|const)/);
    if (importEndMatch) {
      const insertAt = importEndMatch.index! + importEndMatch[0].length;
      apiContent = apiContent.slice(0, insertAt) + "\n" + importStatement + apiContent.slice(insertAt);
    }
  }
