...
import { getHandlersTemplate, getOpenAPITemplate, getRoutesTemplate, getTestTemplate } from "./templates";
...
export function generateModuleFiles(
  ...
  writeFileSync(paths.handlers, getHandlersTemplate({ ...names, schema: schemaName }));
  ...
  console.log(`   - GET    /api/${names.kebab}/:id  - Get ${names.camel} by ID`);
  console.log(`   - POST   /api/${names.kebab}      - Create new ${names.camel}`);
